...
const forwardRuumbaToLinter = (version, { message, location, severity, cop_name: copName }, file, editor) => {
  ...
  const linterMessage = {
    excerpt: hasCopName ? message : `${copName}: ${message}`,
  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        return (offenses || []).map(
          offense => forwardRuumbaToLinter(rubocopVersion, offense, filePath, editor),
        );
      },
    };
  },
